...
page-spec.json + api.md，或直接从详设文档 / 原型描述
...
参考：`src/views/demo/c-form/index.vue`（Liquid Glass 风格）
...
- 状态标签：`<VanTag :type round size="medium">`，颜色通过 `STATUS_MAP` 映射
- 表单：`<VanForm :show-error="false" scroll-to-error>`
- Picker：`<VanPopup position="bottom" round>` + `<VanPicker :columns="OPTIONS">`
