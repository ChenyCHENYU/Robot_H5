<template>
    <van-form v-if="getShow" ref="formRef" class="login-form" @submit="handleRegister">
        <van-field
        ...
        >
            ...
            <template #right-icon>
                <i v-if="switchConfirmPassType" class="i-mdi:eye-outline login-form__icon" />
                <i v-else class="i-mdi:eye-off login-form__icon" />
            </template>
        </van-field>

        <van-field name="policy" class="login-form__field" :rules="getFormRules.policy">
            <template #input>
